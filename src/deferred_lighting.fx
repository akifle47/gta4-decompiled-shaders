...
VS_OutputCorona VS_Corona(VS_InputCorona IN)
{
    VS_OutputCorona OUT;

    float3 centerToVertexDir = IN.Position - IN.Center;
    float radius = length(centerToVertexDir) * 0.707106769;
    float3 viewToCenterDir = IN.Center - gViewInverse[3].xyz;
    float dist = length(viewToCenterDir);

    float4 positionAndAlpha;
    if(dist < radius)
    {
        float3 viewZ = float3(gWorldView[0].z, gWorldView[1].z, gWorldView[2].z);
        float viewDepth = dot(viewToCenterDir, viewZ);

        float alpha = 1.0 - (viewDepth / radius);
        alpha = saturate(alpha * 0.5);

        float behindCam = -viewDepth < 0.1;
        viewZ *= (viewDepth + 0.1);
        float4 shiftedPos = float4(IN.Position - (viewZ * behindCam), alpha);
        float t = -viewDepth < radius;
        positionAndAlpha = lerp(float4(IN.Position, 1), shiftedPos, t);
    }
    else
    {
        const float sampleWeight = 1.0 / 12.0;
        float visibility = 0.0;

        float4 posClip = mul(float4(IN.Center, 1), gWorldViewProj);
        float3 posNdc = posClip.xyz / posClip.w;

        for(int i = 0; i < 12; i++)
        {
            float2 offset = globalScreenSize.zw * poisson12[i] * 2.0;
            float2 samplePos = clamp(posNdc.xy + offset, -1.0, 1.0);
            samplePos = samplePos * float2(0.5, -0.5) + float2(0.5, 0.5);

            float depth = tex2Dlod(GBufferTextureSampler3, float4(samplePos, 0, 0)).x;
            visibility += ((posNdc.z < depth) * sampleWeight);
        }

        positionAndAlpha.w = visibility;
        positionAndAlpha.xyz = IN.Center + (centerToVertexDir * (visibility > 0.01));
    }

    const float colorIntensity = IN.TexCoord.y;
    float4 posClip = mul(float4(positionAndAlpha.xyz, 1), gWorldViewProj);
    OUT.Position = posClip;
    OUT.Color = IN.Color.xyz * positionAndAlpha.w * colorIntensity;
    OUT.TexCoord = float2(IN.TexCoord.x, IN.Color.w);

    return OUT;
}

VS_OutputCorona VS_Corona_R(VS_InputCorona IN)
{
    VS_OutputCorona OUT;

    const float colorIntensity = IN.TexCoord.y;
    float4 posClip = mul(float4(IN.Position, 1), gWorldViewProj);
    OUT.Position = posClip;
    OUT.Color = IN.Color.xyz * colorIntensity;
    OUT.TexCoord = float2(IN.TexCoord.x, IN.Color.w);

    return OUT;
}

VS_OutputCorona VS_Corona_PB(VS_InputCorona IN)
{
    VS_OutputCorona OUT;

    float3 viewPos = gViewInverse[3].xyz;
    viewPos.z += 512;
    
    float3 forwardDir = IN.Center - viewPos;
    float aboveHorizon = forwardDir.z >= 0;
    float3 rightDir = cross(forwardDir, float3(0, 0, 1));
    float3 upDir = cross(forwardDir, rightDir);
    rightDir = normalize(rightDir + 0.000001) * aboveHorizon;;
    upDir = normalize(upDir + 0.000001) * aboveHorizon;

    float3 centerToVertexDir = IN.Position - IN.Center;
    float3 position = IN.Center;
    position += rightDir * centerToVertexDir.x;
    position += upDir * centerToVertexDir.y;

    float3 posView = mul(float4(position, 1.0), gWorldView).xyz;
    posView.z += 512;
    ...
    const float colorIntensity = IN.TexCoord.y;
    float dist = length(centerToVertexDir) * 0.353553385;
    float alpha = saturate(forwardDir.z / dist);
    OUT.Color = IN.Color.xyz * colorIntensity * alpha;
    OUT.TexCoord = float2(IN.TexCoord.x, IN.Color.w);

    return OUT;
}

float4 PS_Corona(VS_OutputCorona IN) : COLOR0
{
    float3 coronaTex = tex2D(gDeferredLightSampler, IN.TexCoord).xyz;
    return float4(coronaTex * IN.Color, 1.0);
}

float4 PS_Corona_PB(VS_OutputCorona IN) : COLOR0
{
    return PS_Corona(IN);
}
...
VS_OutputScreen VS_ScreenTransformT(VS_InputScreen IN)
{
    VS_OutputScreen OUT;
...
float4 PS_RefMipBlur(VS_OutputScreen IN) : COLOR
{
    float3 reflColor = tex2Dlod(gDeferredLightSampler, float4(IN.TexCoord, 0, gDeferredLightColourAndIntensity.w)).xyz;
    reflColor.xyz *= pow(abs(gDeferredLightColourAndIntensity.w), 0.25);

    return float4(reflColor, 1);
}

float4 PS_RefBlend(VS_OutputScreen IN) : COLOR
{
    float3 reflColor = tex2Dlod(gDeferredLightSampler, float4(IN.TexCoord, 0, gDeferredLightColourAndIntensity.w)).xyz;
    return float4(reflColor, gDeferredLightColourAndIntensity.w);
}
...
technique corona
{
    pass p0
    {
        ...
        VertexShader = compile vs_3_0 VS_Corona();
        PixelShader = compile ps_3_0 PS_Corona();
    }
    pass p1
    {
        CullMode = CW;
        ...
        VertexShader = compile vs_3_0 VS_Corona_R();
        PixelShader = compile ps_3_0 PS_Corona();
    }
}
...
technique paraboloid_corona
{
    pass p0
    {
        ...
        VertexShader = compile vs_3_0 VS_Corona_PB();
        PixelShader = compile ps_3_0 PS_Corona_PB();
    }
}
...
technique refMipBlur
{
    pass p0
    {
        ...
        PixelShader = compile ps_3_0 PS_RefMipBlur();
    }
    pass p1
    {
        CullMode = NONE;
        ...
        VertexShader = compile vs_3_0 VS_ScreenTransformT();
        PixelShader = compile ps_3_0 PS_RefBlend();
    }
}
